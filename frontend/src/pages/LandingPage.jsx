...
export default function LandingPage() {
  return (
    <div className="relative min-h-screen overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(59,130,246,0.22),transparent_24%),radial-gradient(circle_at_80%_10%,rgba(96,165,250,0.16),transparent_22%),linear-gradient(180deg,#020617_0%,#071226_52%,#0f172a_100%)]" />
      <motion.div className="absolute left-12 top-24 h-64 w-64 rounded-full bg-blue-500/20 blur-3xl" animate={{ y: [0, 16, 0] }} transition={{ duration: 8, repeat: Infinity }} />
      <motion.div className="absolute right-10 top-36 h-80 w-80 rounded-full bg-cyan-400/10 blur-3xl" animate={{ y: [0, -20, 0] }} transition={{ duration: 10, repeat: Infinity }} />
      <Navbar />
      ...
      <main className="relative">
        <section className="mx-auto grid w-full max-w-7xl gap-12 px-4 pb-20 pt-14 sm:px-6 lg:grid-cols-[1.1fr,0.9fr] lg:px-8 lg:pt-20">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }} className="pt-6">
            <div className="inline-flex items-center gap-3 rounded-full border border-blue-400/20 bg-blue-500/10 px-4 py-2 text-sm text-blue-100 shadow-[0_0_40px_rgba(59,130,246,0.15)]">
            ...
            <h1 className="mt-8 max-w-2xl text-5xl font-black leading-[0.94] tracking-tight text-white sm:text-6xl lg:text-7xl">
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7, delay: 0.1 }} className="relative flex items-center justify-center">
            <Card className="relative w-full max-w-[560px] overflow-hidden p-5">
              ...
              <div className="relative rounded-[24px] border border-white/10 bg-slate-950/75 p-5">
                <div className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-300">
                  ...
                  <span className="rounded-full bg-emerald-500/15 px-3 py-1 text-emerald-300">EVALUATING</span>
                </div>
                    ...
                    <div className="mt-4 space-y-3 text-sm text-slate-300">
                      {['Dashboard', 'Exams', 'Analytics', 'Settings'].map((item, index) => (
                        <div key={item} className={[
                          ...
                          index === 1 ? 'border-blue-400/30 bg-blue-500/15 text-white' : 'border-white/5 bg-white/5',
                        ].join(' ')}>
                          {item}
                        </div>
                      ))}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-[1fr,0.8fr] gap-4">
                      <div className="rounded-[22px] border border-white/10 bg-white/5 p-4">
                        ...
                        <div className="mt-4 rounded-[18px] border border-white/10 bg-slate-950/90 p-4 text-sm text-slate-300">
                          Q1. Newton&apos;s second law states that F = ma. The candidate explains force and acceleration with a neat diagram.
                        </div>
                        <div className="mt-3 flex items-center gap-2 text-xs text-emerald-300">
                          <span className="rounded-full bg-emerald-500/15 px-2 py-1">AI MARKING</span>
              ...
              <motion.div className="absolute -left-4 top-8 rounded-2xl border border-white/10 bg-slate-950/80 px-4 py-3 text-sm shadow-glow" animate={{ y: [0, -8, 0] }} transition={{ duration: 6, repeat: Infinity }}>
                <p className="text-slate-300">147 sheets evaluated</p>
              </motion.div>
              <motion.div className="absolute -right-3 top-24 rounded-2xl border border-white/10 bg-slate-950/80 px-4 py-3 text-sm shadow-glow" animate={{ y: [0, 10, 0] }} transition={{ duration: 5.5, repeat: Infinity }}>
                <p className="text-slate-300">92% OCR Accuracy</p>
              </motion.div>
              <motion.div className="absolute bottom-8 left-14 rounded-2xl border border-white/10 bg-slate-950/80 px-4 py-3 text-sm shadow-glow" animate={{ y: [0, -10, 0] }} transition={{ duration: 6.5, repeat: Infinity }}>
                <p className="text-slate-300">3000+ Questions Generated</p>
              </motion.div>
            </Card>
          </motion.div>
        </section>
        ...
        <section id="features" className="mx-auto w-full max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
          <div className="mb-8 max-w-2xl">
            <p className="text-sm uppercase tracking-[0.3em] text-blue-300/80">Features</p>
            <h2 className="mt-3 text-3xl font-bold text-white sm:text-4xl">Everything faculty teams need in one platform.</h2>
          </div>
        ...
        <section className="mx-auto w-full max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
          <div className="mb-8 max-w-2xl">
            <p className="text-sm uppercase tracking-[0.3em] text-blue-300/80">How it works</p>
            <h2 className="mt-3 text-3xl font-bold text-white sm:text-4xl">A complete workflow in two intuitive modules.</h2>
          </div>
          ...
          <div className="space-y-12">
            ...
            <div>
              <div className="mb-6 flex items-center gap-3">
                <span className="rounded-full bg-blue-500/20 px-3 py-1 text-xs font-bold uppercase tracking-widest text-blue-300">Module 1</span>
                <h3 className="text-xl font-bold text-white">Question Paper Generation</h3>
              </div>
              <div className="grid gap-4 md:grid-cols-3 xl:grid-cols-6">
                {module1Steps.map((step, index) => (
                  <Card key={step} className="relative min-h-[140px] p-4 border-blue-400/10 hover:border-blue-400/30">
                    <div className="text-sm font-bold text-blue-300">0{index + 1}</div>
                    <div className="mt-6 text-sm font-semibold text-white">{step}</div>
            ...
            <div>
              <div className="mb-6 flex items-center gap-3">
                <span className="rounded-full bg-purple-500/20 px-3 py-1 text-xs font-bold uppercase tracking-widest text-purple-300">Module 2</span>
                <h3 className="text-xl font-bold text-white">Answer Sheet Evaluation</h3>
              </div>
              <div className="grid gap-4 md:grid-cols-3 xl:grid-cols-5">
                {module2Steps.map((step, index) => (
                  <Card key={step} className="relative min-h-[140px] p-4 border-purple-400/10 hover:border-purple-400/30">
                    <div className="text-sm font-bold text-purple-300">0{index + 1}</div>
                    <div className="mt-6 text-sm font-semibold text-white">{step}</div>
        ...
        <section className="mx-auto w-full max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
          <div className="mb-8 max-w-2xl">
            <p className="text-sm uppercase tracking-[0.3em] text-blue-300/80">Pricing</p>
            <h2 className="mt-3 text-3xl font-bold text-white sm:text-4xl">Plans that scale from one department to an institution.</h2>
          </div>
          <div className="grid gap-5 lg:grid-cols-3">
            {plans.map((plan) => (
              <Card key={plan.name} className={plan.highlight ? 'border-blue-400/30 bg-blue-500/8' : ''}>
                <p className="text-sm uppercase tracking-[0.3em] text-slate-400">{plan.name}</p>
                <div className="mt-4 flex items-end gap-2">
                  <span className="text-5xl font-black text-white">{plan.price}</span>
                  {plan.price !== 'Free' && <span className="pb-1 text-slate-400">/month</span>}
                </div>
        ...
        <section id="demo" className="mx-auto w-full max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
          <Card className="flex flex-col items-start justify-between gap-6 overflow-hidden bg-gradient-to-r from-blue-500/10 via-white/5 to-cyan-400/10 md:flex-row md:items-center">
            <div>
              <p className="text-sm uppercase tracking-[0.3em] text-blue-300/80">Ready to deploy</p>
              <h2 className="mt-3 text-3xl font-bold text-white">Run the full VISIONGRADE workflow from one dashboard.</h2>
              <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-400">This frontend is designed to feel production-ready from the first render, with routing, realistic mock data, and responsive layouts for every module.</p>
            </div>
