...
const FORMAT_COLOR = {
  ...
  ZIP:  'bg-emerald-500/15 text-emerald-300',
};
  ...
  return (
    <motion.div
      ...
      className={[
        ...
        primary
          ? 'border-blue-400/30 bg-blue-500/15 hover:bg-blue-500/20'
...
export default function FinalReport() {
  ...
  const scoreColor = (s) =>
    s >= 80 ? 'text-emerald-300' : s >= 60 ? 'text-amber-300' : 'text-rose-300';
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mb-5 flex items-center gap-3 rounded-2xl border border-emerald-400/25 bg-emerald-500/[0.07] px-5 py-4">
              ...
              <div className="min-w-0">
                <p className="font-semibold text-white">Evaluation complete — reports ready for download</p>
            ...
            <div className="mb-5 grid grid-cols-3 gap-4">
              {[
                { label: 'Average Score',  value: `${avgScore}%`,                      color: 'text-blue-300'    },
                { label: 'Top Performer',  value: topPerformer?.student ?? '—',         color: 'text-emerald-300' },
                { label: 'Total Evaluated', value: `${evaluations.length} students`,   color: 'text-white'       },
              ].map((s) => (
                <div key={s.label} className="glass-card rounded-2xl border border-white/[0.07] p-4">
            ...
            <div className="grid gap-5 xl:grid-cols-2">
              ...
              <Card>
                ...
                <h3 className="mt-1 text-base font-bold text-white">Batch Score Overview</h3>
                  ...
                  {evaluations.map((ev) => (
                    <div key={ev.id} className="space-y-1.5">
                      <div className="flex items-center justify-between text-xs">
                        <div>
                          <span className="font-semibold text-white">{ev.student}</span>
              ...
              <Card>
                <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-blue-400/70">Downloads</p>
                <h3 className="mt-1 text-base font-bold text-white">Export Files</h3>

                <div className="mt-5 space-y-3">
                  {reports.map((report) => {
                    const Icon = FORMAT_ICON[report.format] ?? FileText;
                    const badgeCls = FORMAT_COLOR[report.format] ?? 'bg-white/[0.08] text-slate-300';
                    return (
                      <div key={report.id} className="flex items-center gap-3 rounded-2xl border border-white/[0.07] bg-white/[0.04] px-4 py-3">
                        ...
                        <button className="flex items-center gap-1.5 rounded-xl border border-white/[0.08] bg-white/[0.05] px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-white/[0.10]">
            ...
            <Card variant="accent" className="mt-5">
              <h3 className="text-sm font-bold text-white">Final Export Options</h3>
              <p className="mt-1 text-xs text-slate-400">Download the complete evaluation package for this exam session.</p>
