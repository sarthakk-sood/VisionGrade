...
const PROVIDER_LABELS = {
  ...
  'groq-llama-3.1-8b':     { label: 'Groq Llama 3.1',        color: 'text-green-400'  },
  'gemini-2.5-flash':      { label: 'Gemini 2.5 Flash',      color: 'text-blue-600'   },
...
export default function TopicReview() {
  ...
  const hydrateProjectFromBackend = useAppStore((s) => s.hydrateProjectFromBackend);

  const completeM1Step          = useAppStore((s) => s.completeM1Step);

  useEffect(() => {
    if (projectId && topics.length === 0 && !topicsLoading) {
      hydrateProjectFromBackend(projectId);
    }
  }, [projectId, topics.length, topicsLoading, hydrateProjectFromBackend]);
  ...
  const providerInfo    = PROVIDER_LABELS[llmProvider] || { label: llmProvider || 'AI', color: 'text-slate-500' };
          ...
          <div className="flex flex-col gap-6 lg:flex-row">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-col items-center justify-center gap-5 py-24 text-center">
                ...
                <div>
                  <p className="text-lg font-bold text-slate-900">Detecting Topics…</p>
                  <p className="mt-1.5 text-sm text-slate-500">AI is analysing your documents. This may take ~60s.</p>
                </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <AnimatePresence>
              {topicsError && (
                <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                  className="mb-5 flex items-start gap-3 rounded-2xl border border-rose-500/20 bg-rose-50 p-4"
                >
                  <AlertCircle className="h-4 w-4 shrink-0 mt-0.5 text-rose-400" />
                  <p className="text-xs text-rose-600">{topicsError}</p>
                </motion.div>
              )}
            </AnimatePresence>
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="w-24 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-xl font-black text-center text-slate-900 outline-none focus:border-blue-400/40 transition"
                  />
                  <button
                    onClick={handleAutoBalance}
                    disabled={selectedCount === 0}
                    className="flex items-center gap-1.5 rounded-xl border border-blue-500/30 bg-blue-50 px-4 py-2.5 text-sm font-semibold text-blue-600 transition hover:bg-blue-100 disabled:opacity-50"
                  >
            ...
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                <button onClick={selectAllTopics}   disabled={allSelected}  className="flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white/[0.03] px-3 py-1.5 text-[10px] font-semibold text-slate-500 transition hover:bg-white/[0.07] hover:text-slate-900 disabled:opacity-40"><CheckSquare className="h-3 w-3" /> Select All</button>
                <button onClick={deselectAllTopics} disabled={noneSelected} className="flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white/[0.03] px-3 py-1.5 text-[10px] font-semibold text-slate-500 transition hover:bg-white/[0.07] hover:text-slate-900 disabled:opacity-40"><Square className="h-3 w-3" /> None</button>
                {projectId && <button onClick={() => detectTopicsFromBackend(projectId)} className="flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white/[0.03] px-3 py-1.5 text-[10px] font-semibold text-slate-500 transition hover:bg-white/[0.07] hover:text-slate-900"><RefreshCw className="h-3 w-3" /> Re-detect</button>}
              </div>
            </div>
            ...
            <div className="grid gap-5 xl:grid-cols-[1.3fr_0.7fr]">
              ...
              <div className="space-y-3">
                {topics.length === 0 ? (
                  <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-slate-200 bg-white/[0.03] py-16 text-center">
                    <Hash className="h-8 w-8 text-slate-700" />
                    <p className="text-sm font-semibold text-slate-500">No topics detected yet</p>
                        ...
                        <div className="flex items-center gap-3 p-4">
                          ...
                          <button
                            ...
                            className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-md border-2 transition-all ${
                              topic.isSelected ? 'border-blue-500 bg-blue-500' : 'border-slate-300 bg-transparent'
                            }`}
                          >
                            <AnimatePresence>
                              {topic.isSelected && (
                                <motion.svg key="chk" initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }}
                                  className="h-3 w-3 text-slate-900" viewBox="0 0 12 12" fill="none">
                          ...
                          <p className="flex-1 min-w-0 truncate text-sm font-semibold text-slate-900">{topic.name}</p>
                          ...
                          {topic.isSelected && (
                            <button
                              ...
                              className="shrink-0 ml-1 rounded-lg p-1 text-slate-500 hover:bg-slate-100 hover:text-slate-900 transition"
                            >
                                ...
                                <div className="space-y-3">
                                  ...
                                  <div className="flex items-center justify-between">
                                    ...
                                    <input
                                      ...
                                      className="w-20 rounded-lg border border-slate-200 bg-slate-50 px-2 py-1.5 text-right text-sm font-bold text-slate-900 outline-none focus:border-blue-400/40"
                                    />
                                  </div>
                                </div>
              ...
              <div className="space-y-4">
                <Card className="sticky top-24">
                  <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-slate-500">Selection Summary</p>
                  <h3 className="mt-1 text-base font-bold text-slate-900">Topic Blueprint</h3>

                  <div className="mt-5 flex items-end gap-2">
                    <span className="text-4xl font-black leading-none text-blue-600">{selectedCount}</span>
                  ...
                  <div className={`mt-4 rounded-xl border p-3 ${
                    ...
                    <div className="flex items-center justify-between">
                      <p className="text-[10px] uppercase font-bold text-slate-500">Allocated Marks</p>
                    ...
                    {selectedTopics.map((t, i) => {
                      ...
                      return (
                        <div key={t.id} className="flex items-center gap-2">
                          <span className={`h-2 w-2 shrink-0 rounded-full ${dot}`} />
                          <span className="flex-1 truncate text-[10px] text-slate-600">{t.name}</span>
                  ...
                  {projectId && (
                    <button
                      ...
                      className="mt-4 w-full flex items-center justify-center gap-2 rounded-xl bg-blue-600/20 border border-blue-500/30 py-2.5 text-xs font-semibold text-blue-600 transition hover:bg-blue-600/30 disabled:opacity-40"
                    >
                ...
                {/* Tip card */}
                <div className="rounded-2xl border border-blue-400/10 bg-blue-50 p-4">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">💡 AI Question Types</p>
