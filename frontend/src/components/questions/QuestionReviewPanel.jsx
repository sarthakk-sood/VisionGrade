  ...
  return (
    <Card>
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-[0.3em] text-blue-300/80">Review Panel</p>
          <h3 className="mt-2 text-xl font-bold text-white">Question quality</h3>
        </div>
        ...
        {questions.map((question) => (
          <div key={question.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-sm font-semibold text-white">{question.type} · {question.marks} marks</p>
                <p className="mt-1 text-sm text-slate-400">{question.text}</p>
              </div>
